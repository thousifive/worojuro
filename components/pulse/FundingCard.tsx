/**
 * FundingCard — pulse item card for category=funding or ipo.
 * Shows "Now hiring likely" badge if company is in user's favorite_companies[].
 */
...
interface FundingCardProps {
  item: PulseItem;
  isFavoriteCompany?: boolean; // cross-ref with user preferences.favorite_companies
  onDismiss?: (id: string) => void;
  onSave?: (id: string) => void;
}

export function FundingCard({ item, isFavoriteCompany, onDismiss, onSave }: FundingCardProps) {
  const summary = item.summary_ai ?? item.summary_raw;
  const isIPO = item.category === 'ipo';

  return (
    <article className="bg-white rounded-xl border border-green-100 p-5">
